...
GLfloat light_position[] = {5.0f,60.0f,30.0f, 1.0f};
...
GLfloat mat_diffuseWHITE_2[] ={2.0,2.0,2.0,0.8};
GLfloat mat_shininessWHITE_2[] ={16.0 * 0.4};
...
// variable d'animation de la texture
float text_trans = 0.0;
...
// Paramètrage de l'éclairage : source 0
...
// Passage à la matrice MODELVIEW
...
// Empecher l'ecriture dans le Color Buffer
glColorMask(0,0,0,0);

// Activer le Stencil-buffer
glEnable(GL_STENCIL_TEST);

// Specifier la fonction test : le test est toujours vrai
glStencilFunc(GL_ALWAYS,1,1);

// Remplacer avec la valeur de reference
...
// Reactivation du Z-buffer
glEnable(GL_DEPTH_TEST);

// Autorisation d'ecriture dans le Color-buffer
glColorMask(1,1,1,1);

// Test d'egalite (avec la valeur "1")
glStencilFunc(GL_EQUAL,1,1);

// Garder les pixels qui reussissent le test d'egalite
glStencilOp(GL_KEEP,GL_KEEP,GL_KEEP);

// Affichage de la scene inverse
glPushMatrix();

// effet miroir
glScalef(1.0f, -1.0f, 1.0f);

// placement de la lumiere dans la scene inversee
glLightfv(GL_LIGHT0, GL_POSITION, light_position);
...
// Desactivation du Stencil-buffer
glDisable(GL_STENCIL_TEST);

// placement de la lumiere dans la scene
glLightfv(GL_LIGHT0, GL_POSITION, light_position);
...
// materiau avec une transparence de 0.8
SetMaterial(mat_specularWHITE_2, mat_ambientWHITE_2, mat_diffuseWHITE_2, mat_shininessWHITE_2);

// Fonction de Blending
glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

// Dessin du sol avec la transparence
plan();

// Desactivation du blending pour afficher le reste de la scene
glDisable(GL_BLEND);
...
void Chrome_inv()
{
// Appliquer aux textures un scalef(1,-1,1) pour les inverser
...
// Clipping de la porte
// Equation du plan
double eqr1[] = {0.0f, 1.0f, 0.0f, 0.0f};

// Activation du plan 0
glEnable(GL_CLIP_PLANE0);

// Parametrage du plan 0
...
glPushMatrix();
	...
	// Creer du Torus
	glutSolidTorus(1,15.0,60,60);
	// Appliquer la texture liquid
	glBindTexture(GL_TEXTURE_2D, texture[6]);
	glPushMatrix();
		// Creer une quadrique
		GLUquadric* params_disk = gluNewQuadric();
		gluQuadricTexture(params_disk, GL_TRUE);
		gluDisk(params_disk,0,15,40,40);
		gluDeleteQuadric(params_disk);
	glPopMatrix();

	// Revenir a la texture 0
	glBindTexture(GL_TEXTURE_2D, 0);
glPopMatrix();

// Desactivation du plan 0
...
// Calcul des faces laterales
  glBindTexture(GL_TEXTURE_2D, texture[1]);   // Choix de la texture.
...
if(profile == segment_2d)
	glTexCoord2f(profile[w*2]/25.0, profile[w*2+1]/25.0);  // 25 = 250/10.0 : taille image/(taille espace noarmalisé*10)
...
/* Generation des coordonnées textures S*/
glEnable(GL_TEXTURE_GEN_S);
/* Generation des coordonnées textures T*/
...
/* Desactivation de la generation des coordonnées S */
glDisable( GL_TEXTURE_GEN_S );
/* Desactivation de la generation des coordonnées T */
...
// Objet chrome inversé :  (sphère)
glNewList(OBJET_6,GL_COMPILE);
glPushMatrix();
...
void tronc_robot()
{
  ...
  glBindTexture(GL_TEXTURE_2D, texture[0]);   // Choix de la texture.
...
{
// Clipping du robot
double eqr2[] = {-1.0f,0.0f,0.0f,0.0f};

// Activation du plan 0
glEnable(GL_CLIP_PLANE1);

// Parametrage du plan
glClipPlane(GL_CLIP_PLANE1, eqr2);

if(base_y > 15 || base_y < -15)
{
...
//tête du robot
glPushMatrix();
  glBindTexture(GL_TEXTURE_2D, texture[3]);   // Choix de la texture.
  glTranslatef(0.0,2.0,0.0);
  glRotatef(-90.0,1.0,0.0,0.0);
...
// Tableau
  ...
  glBindTexture(GL_TEXTURE_2D, texture[2]);   // Choix de la texture.
  ...
  glTexCoord2f(0.8f, 0.0f);  glVertex3f(2.0f,0.0f,-3.0f);
  glTexCoord2f(0.8f, 1.0f);  glVertex3f(2.0f,0.0f,3.0f);
  glTexCoord2f(0.0f, 1.0f);  glVertex3f(-2.0f,0.0f,3.0f);
  glEnd();
  glPopMatrix();
  ...
  glBegin(GL_POLYGON);
  glNormal3f(0.0f,-1.0f,0.0f);
  glTexCoord2f(1.0f, 0.0f);  glVertex3f(-3.0f,0.0f,-0.5f);
  glTexCoord2f(1.0f, 1.0f);  glVertex3f(3.0f,0.0f,-0.5f);
  glTexCoord2f(0.8f, 1.0f);  glVertex3f(3.0f,0.0f,0.5f);
  glTexCoord2f(0.8f, 0.0f);  glVertex3f(-3.0f,0.0f,0.5f);
  glEnd();
  glPopMatrix();
...
glBindTexture( GL_TEXTURE_2D, texture[4] );
GLUquadric* params_globe;
params_globe = gluNewQuadric();
gluQuadricTexture(params_globe,GL_TRUE);
...
// Texture tronc robot
    ...
    image1 = (Image *) malloc(512*512/*sizeof(Image)*/);
    if (image1 == NULL) {
...
// Texture sol
    ...
    // Allocation de l'espace de la texture
    image6 = (Image *) malloc(512*512);
    if (image6 == NULL) {
	printf("Error allocating space for image");
	exit(0);
    }

    if (!ImageLoad("marbre.bmp", image6)) {
	exit(1);
    }
    ...
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT); // Paramétrage de recouvrement
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT); // Paramétrage de recouvrement
...
// Texture liquide
    ...
    // Allocation de l'espace de la texture
    image7 = (Image *) malloc(512*512);
    if (image7 == NULL) {
	printf("Error allocating space for image");
	exit(0);
    }

    if (!ImageLoad("liquid.bmp", image7)) {
	exit(1);
    }
...
// Texture mur
    ...
    // Allocation de l'espace de la texture
    image8 = (Image *) malloc(512*512);
    if (image8 == NULL) {
	printf("Error allocating space for image");
	exit(0);
    }

    if (!ImageLoad("mur.bmp", image8)) {
	exit(1);
    }
    ...
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S, GL_REPEAT); // Paramétrage de recouvrement
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T, GL_REPEAT); // Paramétrage de recouvrement
