...
GLfloat mat_specularRED[] ={1,0.0,0.75,1.0};
GLfloat mat_ambientRED[] ={1,0,0,1.0};
GLfloat mat_diffuseRED[] ={1,0.0,0.0,1.0};
...
GLfloat mat_ambientBLACK[] ={0.0,0,0,1.0};
GLfloat mat_diffuseBLACK[] ={0,0,0,1.0};
GLfloat mat_shininessBLACK[] ={0};
...
/*Profile de l'habitacle*/
GLfloat habitacle_2d[] =
{
0,0,
1,0,
1,1,
...
/*Profile de la base de la voiture*/
GLfloat base_2d[] =
{
0,0,
4,0,
...
double angle_voiture = 0.0;
float voiture_look_x = 0.0;
float voiture_look_z = 5.0;
float vectVoiture_x = 0.0;
float vectVoiture_z = 0.0;
float voiture_pos_x = 0.0;
float voiture_pos_z = 0.0;

float angle_radian = 0.0;

int coffre_ouvert = 45;
int aOuvrir = 1;


//float angleVoiture = 0.0;
...
void rotate_voiture(double angle);
...
//  gluLookAt(0.0,5.0,30.0,0.0,5.0,0.0,0.0,1.0,0.0); // premère partie : robot seul
  ...
  gluLookAt(cam_pos_x,40.0,cam_pos_z,cam_look_x,5.0,cam_look_z,0.0,1.0,0.0);
...
// Déblacement de la voiture
	...
	case GLUT_KEY_UP: /*Marche avant*/
		/*coordonnee premier point*/    
    move_voiture(0.5);
    break; 
        
 	case GLUT_KEY_DOWN:	/*Marche arrière*/
		/*coordonnee premier point*/    
    move_voiture(-0.5);
    break;    
...
extrude(5,base_2d,extrusion_vector);
glPopMatrix();
glEndList();
...
//DessinModele();
extrude(5,habitacle_2d,extrusion_vector2);
glPopMatrix();
glEndList();
...
//DessinModele();
extrude(4,coffre_2d,extrusion_vector2);
glPopMatrix();
glEndList();
...
void move_voiture(double speed)
{
  vectVoiture_x = voiture_look_x - voiture_pos_x;
  vectVoiture_z = voiture_look_z - voiture_pos_z;

  voiture_pos_x += vectVoiture_x * speed;
  voiture_pos_z += vectVoiture_z * speed;

  voiture_look_x += vectVoiture_x * speed;
  voiture_look_z += vectVoiture_z * speed;
}
...
void rotate_voiture(double angle)
{
  /*Convertir angle en radian*/
  angle_radian = angle * (M_PI/180);

  vectVoiture_x = voiture_look_x - voiture_pos_x;
  vectVoiture_z = voiture_look_z - voiture_pos_z;

  // Calculate the sine and cosine of the angle once
  float cosTheta = (float)cos(angle_radian);
  float sinTheta = (float)sin(angle_radian);
 
  //double new_x = sqrt(pow(vectVoiture_x,2) + pow(vectVoiture_z,2)) * cosTheta;
  //double new_z = sqrt(pow(vectVoiture_x,2) + pow(vectVoiture_z,2)) * sinTheta;

  //voiture_look_x = voiture_pos_x + new_x;
  //voiture_look_z = voiture_pos_z + new_z;
  voiture_look_x = (vectVoiture_x)*cosTheta - (vectVoiture_z)*sinTheta + voiture_pos_x;
  voiture_look_z = (vectVoiture_x)*sinTheta + (vectVoiture_z)*cosTheta + voiture_pos_z;
  //printf("coordonnee x point arriere = %f\n", voiture_pos_x);
  //printf("coordonnee x point central = %f\n", voiture_look_x);
  //printf("coordonnee z point arriere = %f\n", voiture_pos_z);
  //printf("coordonnee z point central = %f\n", voiture_look_z);
}
...
void Voiture()
{
  ...
  glTranslatef(voiture_pos_x,0.0,voiture_pos_z);
  glRotatef(angle_voiture,0.0,1.0,0.0);
  ...
  float height = 0.75;
  int distance_roue_z = 13;
  float distance_roue_y = 8.87;
  GLUquadricObj *obj1 = gluNewQuadric();
  gluCylinder(obj1, 1.0, 1, height, 30, 30);
  GLUquadricObj *obj2 = gluNewQuadric();
  gluDisk(obj2,0.0,1.0,30.0,360.0);
  glTranslatef(0.0,0.0,height);
  GLUquadricObj *obj3 = gluNewQuadric();
  gluDisk(obj3,0.0,1.0,30.0,360.0);
  glPopMatrix();

  /*Roue 2*/
  glPushMatrix();
  glColor3f(1.0f, 0.0f, 0.0f);
  glTranslatef(-0.5,1.0,2.5+distance_roue_z);
  glRotatef(90.0,0.0,1.0,0.0);
  glScalef(1.5,1.5,1.5);
  GLUquadricObj *obj4 = gluNewQuadric();
  gluCylinder(obj4, 1.0, 1, height, 30, 30);
  GLUquadricObj *obj5 = gluNewQuadric();
  gluDisk(obj5,0.0,1.0,30.0,360.0);
  glTranslatef(0.0,0.0,height);
  GLUquadricObj *obj6 = gluNewQuadric();
  gluDisk(obj6,0.0,1.0,30.0,360.0);
  glPopMatrix();

  /*Roue 3*/
  glPushMatrix();
  glColor3f(1.0f, 0.0f, 0.0f);
  glTranslatef(distance_roue_y+0.5,1.0,2.5);
  glRotatef(90.0,0.0,1.0,0.0);
  glScalef(1.5,1.5,1.5);
  GLUquadricObj *obj7 = gluNewQuadric();
  gluCylinder(obj7, 1.0, 1, height, 30, 30);
  GLUquadricObj *obj8 = gluNewQuadric();
  gluDisk(obj8,0.0,1.0,30.0,360.0);
  glTranslatef(0.0,0.0,height);
  GLUquadricObj *obj9 = gluNewQuadric();
  gluDisk(obj9,0.0,1.0,30.0,360.0);
  glPopMatrix();

  /*Roue 4*/
  glPushMatrix();
  glColor3f(1.0f, 0.0f, 0.0f);
  glTranslatef(distance_roue_y+0.5,1.0,2.5+distance_roue_z);
  glRotatef(90.0,0.0,1.0,0.0);
  glScalef(1.5,1.5,1.5);
  GLUquadricObj *obj10 = gluNewQuadric();
  gluCylinder(obj10, 1.0, 1, height, 30, 30);
  GLUquadricObj *obj11 = gluNewQuadric();
  gluDisk(obj11,0.0,1.0,30.0,360.0);
  glTranslatef(0.0,0.0,height);
  GLUquadricObj *obj12 = gluNewQuadric();
  gluDisk(obj12,0.0,1.0,30.0,360.0);
  glPopMatrix();
  ...
  glTranslatef(3,3.85,0.3);

  glRotatef(coffre_ouvert*aOuvrir,1,0,0);
  //glRotatef(180,1,0,0);
